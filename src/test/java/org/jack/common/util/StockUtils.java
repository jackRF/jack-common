...
public class StockUtils extends BaseTest {
    ...
    public void testStockTrade() {
        ...
        BigDecimal wPurchase=null;
        BigDecimal bPurchase=null;
        for (;;) {
            stockAccount = new StockAccount(BigDecimal.valueOf(50000));
            ...
            log(stockTrades[0].getTime() + "-" + stockTrades[stockTrades.length - 1].getTime() + "总资产:"
                    + marketValue);
            if(maxMarketValue==null||maxMarketValue.compareTo(marketValue)<0){
                maxMarketValue=marketValue;
                bPurchase=weight.getbPurchase();
            }
            if(weight.getbPurchase().compareTo(BigDecimal.valueOf(0.03))>=0){
                break;
            }
            weight.setbPurchase(weight.getbPurchase().add(BigDecimal.valueOf(0.001)));
        }
        log("bPurchase:"+bPurchase+", maxMarketValue:"+maxMarketValue);
    }
